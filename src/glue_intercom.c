...
static void send_to_intercom(player *p, const char *fmt, ...)
{
 va_list varlist;
 char *oldstack;
...
static void intercom_rejoin_func(player *p)
{
 fvtell_player(NORMAL_T(p), "%s", 
               " Re-Entering intercom mode. Use ^Bhelp intercom^N for help\n"
               " To run ^Bnormal commands^N type /<command>\n"
               " Use '^Bend^N' to ^Bleave^N.\n");
}

static int intercom_command(player * p, char *str, size_t length)
{
 ICTRACE("intercom_command");

 if (MODE_IN_MODE(p, INTERCOM))
   MODE_HELPER_COMMAND();
 else
   if (!*str)
   {
    ...
    CMDS_FUNC_TYPE_RET_CHARS_SIZE_T(&tmp_cmd, intercom_command);
    CMDS_FUNC_TYPE_NO_CHARS(&tmp_rejoin, intercom_rejoin_func);

    if (mode_add(p, "Intercom Mode-> ", MODE_ID_INTERCOM, 0,
                 &tmp_cmd, &tmp_rejoin, NULL))
      fvtell_player(NORMAL_T(p), "%s", 
                    " Entering intercom mode. Use ^Bhelp intercom^N for help\n"
                    " To run ^Bnormal commands^N type /<command>\n"
                    " Use '^Bend^N' to ^Bleave^N.\n");
    else
      fvtell_player(SYSTEM_T(p), 
                    " You cannot enter intercom mode as you are in too many "
                    "other modes.\n");
    return (TRUE);
   }
 
 return (cmds_sub_match(p, str, length,
                        &cmds_sub[CMDS_SUB_OFFSET(CMDS_SECTION_INTERCOM)]));
}

static void intercom_view_commands(player *p)
{
 user_cmds_show_section(p, "intercom");
}
...
void start_intercom()
{
  ...
  sprintf(stack,"-=> %s <=- Intertalker communication server",INTERCOM_NAME);
  stack[MAX_TITLE+9]=0;
  ...
  log ("boot","Forking to boot Intercom Server.");
  intercom_pid = fork();
...
#else
      for (loopa=0;loopa<MAX_CONNECT_SERVERS;loopa++)
	if (connect_server[loopa].fd > 0)
	  close(connect_server[loopa].fd);
#endif
      ...
      break;
    case -1:
      log("boot","Error, failed to fork main server");
      break;
    }
...
#ifdef WALT
  intercom_last=this_second+2400;
#else
...
static void tell_intercom_room(char *str,int room_tag)
{
  ...
  /*Do ourown version of tell_room, cos we need to do intercom-based ignores*/
...
#ifdef WALT
  intercom_last=this_second+1800;
#else
...
void intercom_update_servers(player *p,char *str)
{
  ...
  if (str && *str)
    {
#ifdef VARARGS
      tell_player(p," Requesting talker lists from '%s'.\n",str);
#else
      sprintf(oldstack," Requesting talker lists from '%s'.\n",str);
      stack=end_string(oldstack);
      tell_player(p,oldstack);
      stack=oldstack;
#endif
  ...
  send_to_intercom(p,"%c%s",REQUEST_SERVER_LIST,str &&*str?str:"");
...
{
#ifdef VARARGS
  tell_player(p," Intercom version %s (%s)\n",INTERCOM_VERSION,__DATE__);
#else
  char *oldstack=stack;

  sprintf(stack," Intercom version %s (%s)\n",INTERCOM_VERSION,__DATE__);
  stack=end_string(oldstack);
...
#else
      sprintf(oldstack," You emote: %s%s\n",p->name,str);
      stack=end_string(oldstack);
      tell_player(p,oldstack);
#endif
    }
...
void intercom_site_move(player *p,char *str)
{
...
#ifdef VARARGS  
  tell_player(p," Informing all remote talkers of the address change to "
	      "%s %s\n",site,port);
#else
  sprintf(oldstack," Informing all remote talkers of the address change to "
	  "%s %s\n",site,port);
  stack=end_string(oldstack);
  tell_player(p,oldstack);
  stack=oldstack;
  ...
  send_to_intercom(NULL,"%c%s:%s",WE_ARE_MOVING,site,port);
...
void do_intercom_think(player *p,char *str)
{
...
#ifdef VARARGS
  tell_player(p," You think . o O ( %s )\n",str);
#else
  sprintf(oldstack," You think . o O ( %s )\n",str);
  stack=end_string(oldstack);
  tell_player(p,oldstack);
#endif
  ...
  send_to_intercom(NULL,"%c%c%s:x:x:thinks . o O ( %s )",USER_COMMAND,
