...
/* #define USE_INTERCOM 1 */

#include "main.h"

#ifndef MK_DAYS
# define MK_DAYS(x) (S_DAY*(x))
#endif

time_t now;
sig_atomic_t intercom_panic = FALSE;
sig_atomic_t intercom_reboot = FALSE;

#ifdef USE_INTERCOM

/*Interns*/
char current_name[INTERCOM_PLAYER_NAME_SZ];
...
int inet_fd = -1;
int portnumber = 0;
intercom_talker_list *talker_list_anchor = NULL;
intercom_talker_list *validation_anchor = NULL;
unsigned long intercom_status = -1;
unsigned long job_id = 0;

intercom_job_list *job_anchor = NULL;
intercom_job_list *free_jobs_anchor = NULL;
/* intercom_packet *free_packet_anchor = NULL;
   intercom_packet *talker_packet_anchor = NULL;
*/

intercom_net_usage server_net;
intercom_net_usage total_net;

time_t this_second;
int close_main_socket_now = 0;

char *dynamic_dns = NULL;
...
static int tell_remote_talker(intercom_talker_list *, const char *,...)
    __attribute__ ((format (printf, 2, 3)));
static void tell_personal(const char *,...)
    __attribute__ ((format (printf, 1, 2)));
static void tell_personal_inc_ref(intercom_job_list *, const char *,...)
    __attribute__ ((format (printf, 2, 3)));
...
static void do_close(intercom_talker_list *target)
{
#if 0
 close(target->fd);
...
{
#if 0 /* call socket_com_q_foreach_node() */
 intercom_talker_list *scan;
 ...
 while (scan->next)
 {
  ...
  if (scan->fd>0)
  {
   if (!(scan->flags & INTERCOM_WAITING_CONNECT))
...
static void shutdown_error(const char *filename, const char *string)
{
 vwlog(filename, "%s", string);
...
static intercom_job_list *return_job(char *str)
{
 long unsigned int job_num;
 intercom_job_list *scan;
...
{
 int loopa;
 intercom_job_list *new_job;
 
 /*Add 10 new jobs in*/
 
 for (loopa=0;loopa<10;loopa++)
 {
  new_job = MALLOC(sizeof(intercom_job_list));
...
static intercom_job_list *make_job_entry(void)
{
 intercom_job_list *target_job;
...
static void free_job(intercom_job_list *target_job)
{
 target_job->next->prev = target_job->prev;
 target_job->prev->next = target_job->next;
 
 /*Wipe all info*/
 memset(target_job,0,sizeof(intercom_job_list));
...
{
 job_anchor = MALLOC(sizeof(intercom_job_list));
 free_jobs_anchor = MALLOC(sizeof(intercom_job_list));
...
static packet *add_packet_to_talker(intercom_talker_list *remote_talker)
{
 packet *new_var,*scan;
...
static void free_talker_packets(intercom_talker_list *remote_talker)
{
 packet *target;
...
{
 free_packet_anchor= MALLOC(sizeof(packet));
 
 return;
}
#endif

static intercom_talker_list *match_address(intercom_talker_list *remote_talker)
{
 intercom_talker_list *scan;
...
static void actual_validate_send(intercom_talker_list *known_talker)
{
...
static void request_validation(intercom_talker_list *known_talker)
{
 if (!connect_new_talker(known_talker))
 {
  known_talker->flags |= INTERCOM_VALIDATE_AFTER_CONNECT;
  return;
 }
 
 if (known_talker->flags & INTERCOM_WAITING_CONNECT)
 {
  known_talker->flags |= INTERCOM_VALIDATE_AFTER_CONNECT;
  return;
 }
...
static void rerequest_validation(intercom_talker_list *remote_talker)
{
 intercom_talker_list *known;
...
static void make_unique_but(intercom_talker_list *safe,intercom_talker_list *remote_talker)
{
 char dummy_name[MAX_TALKER_NAME];
 char dummy_abbr[MAX_TALKER_ABBR];
 int loopa;
 int length;
 intercom_talker_list *match;
...
static void make_unique(intercom_talker_list *remote_talker)
{
...
static void parse_hello(intercom_talker_list *remote_talker,char *str)
{
 char *name,*abbr,*port_str,*dynamic,*terminator;
 intercom_talker_list *known;
 struct hostent *hp;
 ...
 if (dynamic && *dynamic)
 {
  ...
  else
  {
   ...
   if (strcmp(numeric_string,remote_talker->addr))
     dynamic=0;
  }
  if (!dynamic)
  {
 ...
 if (!known)
 {
  ...
  {
   /*If it is connected*/
   if (known->fd > 0 || known->fd < -1 ||
       difftime(this_second,known->last_seen) < MK_DAYS(7) ||
  ...
  while (!known && name_match)
  {
   ...
   if (known)
   {
    ...
    if (known->fd < -1 || known->fd > 0 ||
        difftime(this_second,known->last_seen) < 7*S_DAY ||
        known->port!=remote_talker->port)
    {
     known=0;
     name_match++;
 ...
 if (!known)
 {
  ...
  sprintf(oldstack,"%s:%s:%d.%d.%d.%d:%d:B:%s",remote_talker->name,
 ...
 if (known->flags & INTERCOM_FIRST_CONTACT)
 {
  ...
  known->flags &=~ INTERCOM_FIRST_CONTACT;
 }
...
static void reply_validation(intercom_talker_list *known_talker)
{
...
static void parse_validation_request(intercom_talker_list *remote_talker,char *str)
{
...
static void check_validation(intercom_talker_list *remote_talker,char *str)
{
 char *port_str,*validation_str;
 intercom_talker_list *known;
...
static void tell_intercom_room(char *sender,char *remote_site,char *msg)
{
 send_to_talker("%c%s:%s:%s",INTERCOM_ROOM_MESSAGE,sender,remote_site,msg);
 
 return;
}

static void parse_in_user_tell_command(intercom_talker_list *remote_talker,char *str)
{
 char *job_str,*from_name,*msg=0,*oldstack;
 intercom_job_list *this_job;
...
static void parse_in_user_remote_command(intercom_talker_list *remote_talker,char *str)
{
...
static void parse_in_user_examine_command(intercom_talker_list *remote_talker,char *str)
{
...
static void parse_in_user_finger_command(intercom_talker_list *remote_talker,char *str)
{
 char *job_str,*name,*ptr;
 intercom_job_list *this_job;
...
static void parse_in_user_who_command(intercom_talker_list *remote_talker,char *str)
{
 char *job_str,*ptr;
 intercom_job_list *this_job;
...
static void parse_in_user_lsu_command(intercom_talker_list *remote_talker,char *str)
{
 char *job_str,*ptr;
 intercom_job_list *this_job;
...
static void parse_in_user_locate_command(intercom_talker_list *remote_talker,char *str)
{
 char *job_str,*name;
 intercom_job_list *this_job;
...
static void parse_in_user_idle_command(intercom_talker_list *remote_talker,char *str)
{
 char *job_str,*name,*ptr;
 intercom_job_list *this_job;
...
static void parse_in_user_say_command(intercom_talker_list *remote_talker,char *str)
{
 ...
 tell_intercom_room(name,remote_talker->abbr,oldstack);
...
static void parse_in_user_emote_command(intercom_talker_list *remote_talker,char *str)
{
...
static void parse_in_user_command(intercom_talker_list *remote_talker,char *str)
{
 if (!str || !*str)
   return;
...
static void return_good_tell_command(intercom_talker_list *remote_talker,
				     intercom_job_list *this_job)
{
...
static void return_good_remote_command(intercom_talker_list *remote_talker,
				       intercom_job_list *this_job)
{
 if (*(this_job->msg) == 39)
...
static void return_good_command(intercom_talker_list *remote_talker,intercom_job_list *this_job)
{
...
static void send_who_reply_to_server(intercom_talker_list *remote_talker,
				     intercom_job_list *this_job,char *str)
{
...
static void send_string_reply_to_server(intercom_talker_list *remote_talker,
					intercom_job_list *this_job,char *str)
{
 tell_personal("%s",str);
...
static void parse_reply_from_remote(intercom_talker_list *remote_talker,char *str)
{
 intercom_job_list *this_job;
 char *ptr=0,*name;
 char blank_field[2];
 ...
 switch(*str)
 {
  ...
  case TALKER_IGNORED:
    strcpy(current_name,this_job->sender);
    tell_personal(" %s is ignoring %s.",this_job->target,INTERCOM_NAME);
    break;
    
  case TALKER_BLOCKED:
    strcpy(current_name,this_job->sender);
    tell_personal(" %s is blocking %s.",this_job->target,INTERCOM_NAME);
    break;
...
static void get_remote_list(intercom_talker_list *remote_talker,char *str)
{
 char *name,*addr,*abbr,*port_str,*ptr,*oldstack,*dynamic,*terminator;
 ...
 if (!strcasecmp(name,INTERCOM_NAME) || 
     !strcasecmp(abbr,INTERCOM_ABBR))
   return;
 ...
 oldstack=stack;
 sprintf(oldstack,"%s:%s:%d.%d.%d.%d:%s:F:%s",name,abbr,num[0],
         num[1],num[2],num[3],port_str,dynamic&&*dynamic?dynamic:"");
 
 stack=end_string(oldstack);
 if (add_new_talker(oldstack,0))
 {
  sprintf(oldstack,"Talker '%s' just informed us of talker '%s'. Added "
          "to the database as a banished talker.",remote_talker->name,
          name);
  stack=end_string(oldstack);
  tell_talker_su(oldstack);
 }
...
static void send_server_list(intercom_talker_list *remote_server)
{
 intercom_talker_list *scan;
 ...
 {
  scan=scan->next;
  if (scan->fd>0 && scan->validation>0 && !(scan->flags & INTERCOM_WAITING_CONNECT))
  {
...
static void remote_has_barred(intercom_talker_list *remote_talker)
{
 I_SHUTDOWN(remote_talker->fd,2);
...
static void do_room_look(intercom_talker_list *remote_talker,char *str)
{
 intercom_job_list *this_job;
...
static void return_room_look(intercom_talker_list *remote_talker,char *str)
{
...
static void informed_move(intercom_talker_list *remote_talker,char *str)
{
 int dots,all_num;
 ...
 if (all_num)
 {
  ...
  if (hp)
    strcpy(remote_talker->addr,(char *)hp->h_name);
  else
...
static void do_user_action(intercom_talker_list *remote_talker,char *str)
{
 switch(*str)
 {
  case ENTER_ROOM:
...
#if 0
static void parse_remote_talker_input(intercom_talker_list *remote_talker,int validated)
{
 char c;
 int chars_left;
 char *oldstack,*ptr;
 ...
 /*Firstly, if we were waiting for a connect, this means it failed if we get
   here*/
 if (remote_talker->flags & INTERCOM_WAITING_CONNECT)
 {
  do_close(remote_talker);
  
  remote_talker->flags &= ~INTERCOM_WAITING_CONNECT;
  remote_talker->flags &= ~INTERCOM_HELLO_AFTER_CONNECT;
...
static int get_new_talker_connection(void)
{
 ...
 struct sockaddr_in sa;
 int new_fd,dummy=0;
 intercom_talker_list *new_var,*scan;
 struct hostent *hp;
 ...
 new_var = MALLOC(sizeof(intercom_talker_list));
 scan->next=new_var;
 new_var->fd=new_fd;
 ...
 hp=gethostbyaddr((char *)&(sa.sin_addr.s_addr),
                  sizeof(sa.sin_addr.s_addr),AF_INET);
 if (hp)
   strcpy(new_var->addr,(char *)hp->h_name);
 else
...
int tell_remote_talker(intercom_talker_list *remote_talker,const char *fmt,...)
{
 va_list varlist;
 char *oldstack,*ptr;
 int fail=0;
...
static void send_hello(intercom_talker_list *remote_talker)
{
 if (dynamic_dns && *dynamic_dns)
   tell_remote_talker(remote_talker,"%c%s:%s:%d:%s",HELLO_I_AM,
                      INTERCOM_NAME,INTERCOM_ABBR,portnumber,dynamic_dns);
 else
   tell_remote_talker(remote_talker,"%c%s:%s:%d",HELLO_I_AM,
                      INTERCOM_NAME,INTERCOM_ABBR,portnumber);
 
 return;
}

int connect_new_talker(intercom_talker_list *new_link)
{
 char numeric_string[MAX_INET_ADDR];
 struct in_addr inet_address;
 struct sockaddr_in sa;
 ...
 if (new_link->flags & INTERCOM_WAITING_CONNECT)
   new_link->flags &= ~INTERCOM_WAITING_CONNECT;
 if (new_link->flags & INTERCOM_HELLO_AFTER_CONNECT)
   new_link->flags &= ~INTERCOM_HELLO_AFTER_CONNECT;
 if (new_link->flags & INTERCOM_VALIDATE_AFTER_CONNECT)
   new_link->flags &= ~INTERCOM_VALIDATE_AFTER_CONNECT;
 
 if (new_link->fd==BARRED || new_link->fd==P_BARRED ||
     new_link->flags & INTERCOM_INVIS)
   return 0;
...
static void create_inet_socket(char *str)
{
 ...
 if (portnumber < 1)
 {
  log("intercom","Talker sent port under 1 (%d), Re-requesting.",
      portnumber);
  request_port_from_talker();
  return;
 }
 ...
 while (scan->next)
 {
  ...
  if (connect_new_talker(scan))
    ...
    scan->flags |= INTERCOM_HELLO_AFTER_CONNECT;
 }
...
void tell_personal_inc_ref(intercom_job_list *this_job,const char *fmt,...)
{
 char *oldstack,*str;
 va_list varlist;
...
{
 intercom_talker_list *scan,*new_talker,*prev;
 char *name,*abbr,*addr,*port,*status,*oldstack,*ptr,*last_seen,*dynamic;
 char numeric_string[MAX_INET_ADDR];
 int all_num,dots;
 struct hostent *hp;
 struct in_addr inet_address;
 ...
 new_talker = MALLOC(sizeof(intercom_talker_list));
 ...
 {
  new_talker->fd=P_BARRED;
  new_talker->flags |= INTERCOM_FIRST_CONTACT;
 }
 else if (*status=='I')
 {
  new_talker->fd=P_BARRED;
  new_talker->flags |= INTERCOM_INVIS;
 }
 ...
 {
  if (new_talker->flags & INTERCOM_INVIS &&
      system_time > new_talker->last_seen + (14515200))
  {
   free(new_talker);
   return 0;
  }
 ...
 if (all_num)
 {
  ...
  /*If we found it, use it*/
  if (hp)
    strcpy(new_talker->addr,(char *)hp->h_name);
  else
 ...
 if (verbose!=-1 && new_talker->fd!=P_BARRED)
 {
  if (connect_new_talker(new_talker))
    ...
    new_talker->flags |= INTERCOM_HELLO_AFTER_CONNECT;
 }
 
 return 1;
}

intercom_talker_list *match_talker_name(char *str)
{
 intercom_talker_list *scan;
...
intercom_talker_list *match_talker_abbr_absolute(char *str)
{
 intercom_talker_list *scan;
...
intercom_talker_list *match_talker_abbr_pattern(char *str)
{
 intercom_talker_list *match=0;
 intercom_talker_list *scan;
...
static void read_talker_database(void)
{
 ...
 talker_list_anchor = MALLOC(sizeof(intercom_talker_list));
 validation_anchor = MALLOC(sizeof(intercom_talker_list));
...
static void show_all_links(int hidden,int up_only)
{
 ...
 if (!hidden && inet_fd>1)
 {
  ...
  {
   if (!(scan->flags & INTERCOM_INVIS) && scan->fd > 0 &&
       scan->validation > 0 && !(scan->flags & INTERCOM_WAITING_CONNECT))
   {
    ucount++;
 ...
 if (up_only)
   ...
   while (scan && (stack+200 < stack_start+INTERCOM_STACK))
   {
    ...
    else
    {
     ...
     scan->fd<1 ||
     scan->flags & INTERCOM_WAITING_CONNECT ||
     scan->validation<1))
       show=0;
    ...
    if (show)
    {
     ...
     if (scan->fd == BARRED)
       ...
       strcpy(stack,"RBLK");
     else if (scan->flags & INTERCOM_INVIS)
       strcpy(stack,"INVS");
     else if (scan->fd < 1)
       strcpy(stack,"DOWN");
     else if (scan->flags & INTERCOM_WAITING_CONNECT)
       strcpy(stack,"WAIT");
 ...
 buffer = MALLOC(strlen(oldstack)+2);
...
static void show_all_links_short(void)
{
 ...
 while (scan)
 {
  ...
  if (scan->fd>0 && !(scan->flags & INTERCOM_INVIS) && 
      scan->validation>0 && !(scan->flags & INTERCOM_WAITING_CONNECT))
  {
   while (*stack)
   {
...
{
 intercom_talker_list *open_site;
 char *oldstack;
 ...
 if (connect_new_talker(open_site))
   ...
   open_site->flags |= INTERCOM_HELLO_AFTER_CONNECT;
...
static void unbar_talker(char *str)
{
 ...
 if (open_site->flags & INTERCOM_INVIS)
 {
...
{
 intercom_talker_list *close_site;
 char *oldstack;
...
static void do_out_user_command_locate(char *target_name)
{
 ...
 if (strnlen(target_name, INTERCOM_PLAYER_NAME_SZ) == INTERCOM_PLAYER_NAME_SZ)
   target_name[INTERCOM_PLAYER_NAME_SZ - 1] = 0;
...
static void do_out_user_command_room(char command_style,char *msg)
{
 ...
 while (scan)
 {
  if (scan->fd>0 && !(scan->flags & INTERCOM_INVIS) && 
      scan->validation>0 && !(scan->flags & INTERCOM_WAITING_CONNECT))
  {
   tell_remote_talker(scan,"%c%c%s:%s", INTERCOM_USER_COMMAND,
...
static void do_out_user_command(char *str)
{
 ...
 intercom_talker_list *destination_talker;
 intercom_job_list *this_job;
 char *command_style;
 ...
 if (destination_talker->fd < 1 ||
     destination_talker->flags & INTERCOM_WAITING_CONNECT)
 {
...
static void return_command_with_string(intercom_talker_list *remote_server,
				       intercom_job_list *this_job,char *name,
...
{
 intercom_job_list *this_job;
 intercom_talker_list *remote_server;
 char *name,*ptr;
 char blank_field[2];
...
{
 char *oldstack;
 intercom_talker_list *current;
 
 oldstack=stack;
 
 if (!(intercom_status & BAR_ALL_CONNECTIONS))
 {
  tell_personal(" You close %s to external messages.",INTERCOM_NAME);
...
static void open_all_links(void)
{
 ...
 if (intercom_status & BAR_ALL_CONNECTIONS)
 {
  tell_personal(" You open %s to external messages.",INTERCOM_NAME);
 ...
 while (current->next)
 {
  ...
  if (current->fd!=BARRED && current->fd!=P_BARRED && current->fd<1)
  {
   if (connect_new_talker(current))
     ...
     current->flags |= INTERCOM_HELLO_AFTER_CONNECT;
  }
 }
...
{
 FILE *fp;
 intercom_talker_list *scan,*prev;
 char address[MAX_INET_ADDR];
  ...
  if (scan->fd!=NO_SYNC_TALKER)
  {
   ...
   if (scan->flags & INTERCOM_INVIS)
     fprintf(fp,"%s:%s:%s:%d:I:%ld:%s\n",scan->name,scan->abbr,address,
             scan->port,scan->last_seen,scan->dynamic);
   else if (scan->fd!=P_BARRED)
     fprintf(fp,"%s:%s:%s:%d:O:%ld:%s\n",scan->name,scan->abbr,address,
             scan->port,scan->last_seen,scan->dynamic);
   else if (scan->flags & INTERCOM_FIRST_CONTACT)
...
{
 char *new_var=0;
 intercom_talker_list *scan,*target;
 ...
 if (scan->flags & INTERCOM_FIRST_CONTACT)
   scan->flags &=~ INTERCOM_FIRST_CONTACT;
...
{
 char *new_var=0,*ptr;
 intercom_talker_list *target,*scan;
 ...
 if (connect_new_talker(target))
   ...
   target->flags |= INTERCOM_HELLO_AFTER_CONNECT;
...
static void unhide_entry(char *str)
{
 ...
 if (!(target->flags & INTERCOM_INVIS))
 {
  tell_personal(" %s doesnt appear to be invisible.",target->name);
  return;
 }
 
 target->flags &= ~INTERCOM_INVIS;
...
static void request_server_list(char *str)
{
 intercom_talker_list *scan;
 ...
 {
  scan=scan->next;
  if (!str || !*str || !strcasecmp(str,scan->abbr) ||
      !strcasecmp(str,scan->name))
    if (scan->fd>0)
...
static void request_stats(char *str)
{
 ...
 else if (!strcasecmp(str,INTERCOM_NAME) || !strcasecmp(str,INTERCOM_ABBR))
 {
  stats = &server_net;
 ...
 if (target)
 {
  ...
  if (target->fd == BARRED)
    ...
    strcpy(stack,"Down");
  else if (target->flags & INTERCOM_WAITING_CONNECT)
...
{
 intercom_job_list *this_job;
 intercom_talker_list *remote_talker=0;
 char *job_str,*msg=0;
...
int main(int argc, char *argv[])
{
 ...
 intercom_talker_list *current_talker,*prev;
 time_t last_click, now;
 
 now = this_second = time(NULL);
 ...
 if (strchr(INTERCOM_NAME,':'))
 {
  log("intercom","INTERCOM_NAME may NOT have a : in it. Not booting.");
  exit (-1);
 }
 if (strchr(INTERCOM_ABBR,':'))
 {
  log("intercom","INTERCOM_ABBR may NOT have a : in it. Not booting.");
  exit (-1);
 }
 
 init_signals(); 
 
 system_time = this_second;
 ping_time = system_time+600;
  ...
  if (inet_fd>-1)
  {
   ...
   while (current_talker->next)
   {
    ...
    if (current_talker->fd > 0)
    {
     ...
     if (current_talker->flags & INTERCOM_WAITING_CONNECT)
            ...
            connects_this_loop==0)
    {
     ...
     if (connect_new_talker(current_talker))
       ...
       current_talker->flags |= INTERCOM_HELLO_AFTER_CONNECT;
    }
   }
   ...
   while (current_talker->next)
   {
    ...
    if (current_talker->fd > 0)
    {
         ...
         FD_ISSET(current_talker->fd,&connect_list))
     {
      ...
      current_talker->flags &= ~INTERCOM_WAITING_CONNECT;
      
      /*If we need a hello sent, send it*/
      if (current_talker->flags & INTERCOM_HELLO_AFTER_CONNECT)
      {
       current_talker->flags &= ~INTERCOM_HELLO_AFTER_CONNECT;
       
       send_hello(current_talker);			
      }
      if (current_talker->flags & INTERCOM_VALIDATE_AFTER_CONNECT)
      {
       current_talker->flags &= ~INTERCOM_VALIDATE_AFTER_CONNECT;
...
#ifdef WALT
 log_all_mallocs("intercom_mallocs");
#endif
...
int main(void)
{
 fprintf(stderr, "%s", " The intercom hasn't been compiled.\n");
 exit (EXIT_FAILURE);
}
#endif
