...
int main(void)
{
 socket_com_base *base = socket_com_spawn_socket("./test2", NULL);
 ...
 while (!base->read_bad && !base->write_bad && !base->malloc_bad)
 {
  ...
  socket_poll_update_all(20000);

  socket_com_read_call(base);
  size = socket_com_read_gather(base, '\n');

  if (!size)
    continue;

  socket_com_write_fmt(base, "test1:%08u:%8u %n", ++count,
                       (unsigned)getpid(), &len);
