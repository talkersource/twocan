...
static void internal_signal_big_crash(S_IGNAL_CRASH_PARAMS(s_ig_num, con))
{
 if (!sys_flag.panic)
 {
  S_IGNAL_CRASH_INFO("address");
 }

 shutdown_error("Crash signal received (%d) -- address.log -- might help.\n",
                s_ig_num);
}
...
static void internal_signal_reboot(int s_ig_num)
{
 ...
 CONST_COPY_STR_LEN(shutdown_msg,
                    "** Shutdown via. SIGUSR1, asked to shutdown **");
 
 sys_flag.shutdown = TRUE;
}

static void internal_signal_backup(int s_ig_num)
{
 IGNORE_PARAMETER(s_ig_num);
...
static void internal_signal_reload(int s_ig_num)
{
 IGNORE_PARAMETER(s_ig_num);
 
 sys_flag.reload_run = TRUE;
}
...
void init_signals(void)
{
 ...
 sa.sa_handler = internal_signal_backup;
 S_IGNAL_SETUP(SIGUSR2);

 sa.sa_handler = internal_signal_reload;
 ...
 do
 {
  ...
  { /* this should be impossible */
   vwlog("error", "sigaltstack error: %d %s\n", errno, strerror(errno));
   break;
  }
